...
import { MenuItem, MenuSection as MenuSectionType, CartItem, AdditionGroup, AdditionItem } from './types';
...
const App: React.FC = () => {
  ...
  const [additionsCrepe, setAdditionsCrepe] = useState<AdditionGroup>(() => {
    const saved = localStorage.getItem('atyab_additions_crepe');
  ...
  const saveMenuChanges = () => {
    localStorage.setItem('atyab_menu_data', JSON.stringify(menuData));
    localStorage.setItem('atyab_additions_pizza', JSON.stringify(additionsPizza));
    localStorage.setItem('atyab_additions_crepe', JSON.stringify(additionsCrepe));
  ...
  const sendOrderToWhatsApp = () => {
    ...
    cart.forEach((item, idx) => {
      ...
      if (item.addons.length > 0) message += `   ➕ إضافات: ${item.addons.map(a => `${a.quantity}x ${a.name} (+${a.price * a.quantity}ج)`).join(' + ')}\n`;
    ...
    setTimeout(() => { setIsSending(false); setIsCheckoutOpen(false); setCart([]); }, 2000);
  };

  const additionsGroup = (() => {
    if (!selectedItem) return null;
    const sectionId = selectedItem.section.id;
    if (['fatayer-savory', 'pizza-oriental', 'pizza-italian'].includes(sectionId)) return additionsPizza;
    if (['crepe-savory', 'rolls', 'syrian'].includes(sectionId)) return additionsCrepe;
    return null;
  })();
      ...
      <main className="max-w-2xl mx-auto px-5 py-8 pb-48">
        ...
        {isAdmin && (
          <div className="space-y-10 mt-10 animate-slide-up">
            <div className="bg-zinc-100 dark:bg-zinc-800/50 p-6 rounded-[2rem] border-2 border-dashed border-yellow-600/30">
               <h3 className="text-center font-black text-yellow-600 mb-6 uppercase tracking-widest text-sm">تعديل أسعار الإضافات</h3>
               <MenuSection 
          ...
          <div className="w-full max-w-[340px] bg-white dark:bg-zinc-900 rounded-[2.5rem] overflow-hidden shadow-2xl animate-slide-up border border-white/10 flex flex-col max-h-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-8 bg-zinc-50 dark:bg-white/[0.05] border-t border-zinc-100 dark:border-white/5">
              <div className="flex items-center gap-4">
                ...
                <button onClick={addToCart} className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-black font-black py-4 rounded-xl text-lg shadow-md active:scale-[0.97] transition-all">إضافة</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/90 backdrop-blur-lg px-8 py-24 md:py-32" onClick={() => setIsCheckoutOpen(false)}>
          <div className="w-full max-w-[340px] bg-white dark:bg-zinc-900 rounded-[2.5rem] overflow-hidden shadow-2xl animate-slide-up relative flex flex-col max-h-full border border-white/10" onClick={e => e.stopPropagation()}>
                    ...
                    <div key={item.id} className="flex flex-col gap-3 relative border-b last:border-0 border-zinc-100 dark:border-white/5 pb-5 last:pb-0">
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-black text-base tabular-nums text-yellow-600">{(item.price + item.addons.reduce((s,a)=>s+(a.price * a.quantity),0)) * item.quantity}ج</span>
                        </div>
                        <div className="text-right flex-1 pr-3">
                           ...
                           <h4 className="font-black text-xs text-zinc-800 dark:text-zinc-100 leading-tight">{item.name} <span className="text-yellow-600 ml-1">x{item.quantity}</span></h4>
            ...
            <div className="p-10 bg-white dark:bg-zinc-900 border-t-2 border-zinc-100 dark:border-white/5">
              <button disabled={isSending} onClick={sendOrderToWhatsApp} className={`w-full ${isSending ? 'bg-zinc-400' : 'bg-[#25D366] shadow-lg'} text-white font-black py-5 rounded-xl text-lg active:scale-[0.98] flex items-center justify-center gap-3 transition-all`}>
